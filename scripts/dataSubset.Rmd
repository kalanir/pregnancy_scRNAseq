...
ribo = c(grep("^RPL", rownames(cluster1.markers), value = T),grep("^RPS", rownames(cluster1.markers), value = T))
mito = c(grep("^MT-", rownames(cluster1.markers), value = T), grep("^MTR", rownames(cluster1.markers), value = T))

rpl = cluster1.markers[rownames(cluster1.markers) %like% "^RPL", ]
rps = cluster1.markers[rownames(cluster1.markers) %like% "^RPS", ]
mtr = cluster1.markers[rownames(cluster1.markers) %like% "^MT-", ]
combinedRM = rbind(rpl, rps, mtr)
View(combinedRM)
View(mtr)
length(ribo) #31
length(mito) #7
length(rownames(cluster1.markers)) #81
view(ribo)
(31+7)/81
head(cluster1.markers, n = 10)
...
#https://satijalab.org/seurat/v2.4/pbmc3k_tutorial.html
```{r}
